...
def process_client_data(strategy):
    ...
    for round in strategy.client_accuracy_history:
        ...
        for client in data[round]:
            ...
            client['normalised_distance'] = strategy.client_cluster_normalised_distance_history[client['cid']][round - 1]
...
def plot_client_data_per_round(data, num_rounds):
    """
        Plots each client's training data
    ...
    rows = 3
    cols = num_rounds // rows + (num_rounds % rows > 0)
...
def plot_accuracy_history(data):
    ...
    for round_number, clients in data.items():
        ...
        average_accuracies.append(total / num_clients)
    ...
    plt.plot(rounds, average_accuracies, marker='o', label='Average accuracy')
    plt.xlabel('Round Number')
...
def plot_distance_history(data, key):
    """Plot the history of distances over rounds."""
    ...
    for round_number, clients in data.items():
        for client in clients:
            ...
            client_distances[cid].append((round_number, distance))
    ...
    plt.xlabel('Round #')
    plt.ylabel(key)
    plt.title(f'{key} of each client from the cluster center')
    plt.legend()
    plt.show()


def plot_reputation_history(data):
    """Plot reputation history over rounds."""
    ...
    for round_number, clients in data.items():
        for client in clients:
            cid = client['cid']
            reputation = client['reputation'][0] if isinstance(client['reputation'], np.ndarray) else client['reputation']
